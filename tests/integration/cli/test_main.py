...
def test_main_invalid_target_missing_colon(capsys: pytest.CaptureFixture[str]) -> None:
    assert main(["nosuchtarget"]) == ExitCode.USAGE
...
def test_main_invalid_target_file_missing(
    ...
    assert main([f"{missing_path}:main"]) == ExitCode.USAGE
    captured = capsys.readouterr()
...
def test_main_invalid_target_attribute_missing(
    ...
    assert main(["mod:missing"]) == ExitCode.USAGE
    captured = capsys.readouterr()
...
def test_main_file_target_missing_symbol_mentions_file_path(
    ...
    assert main([f"{module_path}:missing"]) == ExitCode.USAGE
    captured = capsys.readouterr()
...
def test_main_rejects_parser_target(
    ...
    _write_module(
        ...
        "\n".join(
            [
                ...
                "    return msg",
                "",
                "parser = Interfacy(print_result=False, allow_args_from_file=True)",
    ...
    assert main([f"{module_path}:parser", "echo", "hi"]) == ExitCode.USAGE
    captured = capsys.readouterr()
...
def test_main_rejects_command_group_target(
    ...
    assert main([f"{module_path}:cli", "app", "echo", "hi"]) == ExitCode.USAGE
    captured = capsys.readouterr()
    assert "function, class, or class instance" in captured.err
...
def test_main_config_can_load_plugins_before_module_hook(
    ...
    _write_module(
        ...
        "\n".join(
            [
                "import plugin_cli_config_mod",
                "",
                "def echo(*, name: str) -> str:",
                "    return name",
                "",
                "def configure_interfacy(parser):",
                "    if not plugin_cli_config_mod.configured:",
