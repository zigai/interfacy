...
def test_argparser_respects_allow_args_from_file(tmp_path: Path) -> None:
    ...
    literal_parser = Interfacy(backend="argparse", allow_args_from_file=False)
    literal_parser.add_command(echo)

    assert parser.invoke(args=[f"@{args_file}"]) == ("logs", "backup.zip", True)
    assert literal_parser.invoke(args=[f"@{args_file}"]) == f"@{args_file}"
