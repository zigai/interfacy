...
@dataclass
class SettingsWithNestedDefault:
    nested: NestedSettings = field(default_factory=lambda: NestedSettings(retries=3))
...
def collect_items(items: list[SingleFieldItem]) -> list[SingleFieldItem]:
    return items


def collect_boolean_items(items: list[BooleanItem]) -> list[BooleanItem]:
    return items


def load_settings(settings: SettingsWithNestedDefault) -> SettingsWithNestedDefault:
    return settings


def load_config(config: ConfigWithDerivedField) -> ConfigWithDerivedField:
    return config


def test_list_of_single_field_models_is_reconstructed() -> None:
    result = Argparser(sys_exit_enabled=False).run(collect_items, args=["1", "2"])

    assert result == [SingleFieldItem(1), SingleFieldItem(2)]
    assert all(isinstance(item, SingleFieldItem) for item in result)


def test_boolean_fields_in_model_containers_consume_values() -> None:
    result = Argparser(sys_exit_enabled=False).run(
        collect_boolean_items,
        args=["true", "false"],
    )

    assert result == [BooleanItem(enabled=True), BooleanItem(enabled=False)]


def test_nested_model_default_does_not_make_its_leaf_flags_required() -> None:
    parser = Argparser(sys_exit_enabled=False)
    parser.add_command(load_settings)

    command = parser.build_parser_schema().commands["load-settings"]
    assert [(argument.name, argument.required) for argument in command.parameters] == [
        ("settings.nested.retries", False)
    ]
    assert parser.run(args=[]) == SettingsWithNestedDefault()


def test_dataclass_init_false_field_is_not_exposed_or_passed_to_constructor() -> None:
    parser = Argparser(sys_exit_enabled=False)
    parser.add_command(load_config)

    command = parser.build_parser_schema().commands["load-config"]
    assert [argument.name for argument in command.parameters] == ["config.name"]

    result = parser.run(args=["--config.name", "Ada"])
    assert result == ConfigWithDerivedField("Ada")
    assert result.normalized_name == "ada"
