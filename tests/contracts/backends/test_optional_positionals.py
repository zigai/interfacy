...
def test_optional_scalar_positional_can_be_omitted_or_provided(
    ...
    assert parser.invoke(args=["pyproject.toml"]) == "pyproject.toml"
...
def test_validate_config_regression_uses_optional_config_positional(
    ...
    assert parser.invoke(args=["validate"]) is None
    assert parser.invoke(args=["validate", "custom.toml"]) == "custom.toml"
...
@pytest.mark.parametrize("parser", ["argparse_req_pos", "click_req_pos"], indirect=True)
def test_defaulted_scalar_positional_preserves_default(parser: Interfacy) -> None:
    parser.add_command(defaulted_profile)

    assert parser.invoke(args=[]) == "default"
    assert parser.invoke(args=["custom"]) == "custom"


@pytest.mark.parametrize("parser", ["argparse_req_pos", "click_req_pos"], indirect=True)
def test_defaulted_list_positional_preserves_default(parser: Interfacy) -> None:
    parser.add_command(defaulted_values)

    assert parser.invoke(args=[]) == [5]
    assert parser.invoke(args=["1", "2"]) == [1, 2]
